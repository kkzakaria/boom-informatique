import { cn } from '@/lib/utils'
import { StarRatingDisplay } from './StarRatingInput'
import type { RatingStats } from '@/server/reviews'

interface RatingDistributionProps {
  stats: RatingStats
  className?: string
}

export function RatingDistribution({ stats, className }: RatingDistributionProps) {
  const { average, count, distribution } = stats

  return (
    <div className={cn('flex flex-col gap-4 sm:flex-row sm:gap-8', className)}>
      {/* Average rating */}
      <div className="flex flex-col items-center justify-center gap-2">
        <span className="text-5xl font-bold text-[--text-primary]">
          {average.toFixed(1)}
        </span>
        <StarRatingDisplay rating={average} showCount={false} size="md" />
        <span className="text-sm text-[--text-muted]">
          {count} avis
        </span>
      </div>

      {/* Distribution bars */}
      <div className="flex-1 space-y-2">
        {([5, 4, 3, 2, 1] as const).map((star) => {
          const starCount = distribution[star]
          const percentage = count > 0 ? (starCount / count) * 100 : 0

          return (
            <div key={star} className="flex items-center gap-2">
              <span className="w-8 text-sm text-[--text-muted] text-right">
                {star}
              </span>
              <svg
                className="h-4 w-4 text-warning flex-shrink-0"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
              </svg>
              <div className="flex-1 h-2 rounded-full bg-surface-200 dark:bg-surface-700 overflow-hidden">
                <div
                  className="h-full bg-warning rounded-full transition-all duration-300"
                  style={{ width: `${percentage}%` }}
                />
              </div>
              <span className="w-8 text-sm text-[--text-muted]">
                {starCount}
              </span>
            </div>
          )
        })}
      </div>
    </div>
  )
}
